...
// While the Red-Nosed Reindeer nuclear fusion/fission plant appears to contain no sign of the Chief Historian, the engineers there run
//up to you as soon as they see you. Apparently, they still talk about the time Rudolph was saved through molecular synthesis from a
//single electron.
...
const readDataFromFile = (file) => {
  ...
  const dataArray = [];
  const dataDivided = data.split(/\r?\n|\r|\n/g);

  dataDivided.forEach((line) => {
    if (line.trim() !== "") {
      dataArray.push(line.split(" ").map(Number));
    }
  });
...
const defineArrays = () => {
  ...
  for (const element of data) {
    if (isAscending(element) || isDescending(element)) {
...
const sortedArrays = defineArrays();

const checkIfCombinationIsSafe = () => {
  ...
  for (const array of sortedArrays) {
    ...
    for (let j = 1; j < array.length; j++) {
      const difference = Math.abs(array[j] - array[j - 1]);
...
const safeCombinations = checkIfCombinationIsSafe();
const solution = safeCombinations.length;
...
// Now, the same rules apply as before, except if removing a single level from an unsafe report would make it safe, the report instead
//counts as safe.
